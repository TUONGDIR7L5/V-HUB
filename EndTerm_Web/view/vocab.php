...
<body>
  ...
  <div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <ul class="nav nav-tabs border-0" id="vocab-tabs">
            <li class="nav-item">
    ...
    <div class="row g-4">
        <div class="container my-5">
            <!-- Danh sách từ vựng sẽ được cập nhật động -->
            <div class="row row-cols-1 row-cols-md-3 g-4" id="vocab-list">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Tải dữ liệu ban đầu
    loadVocabulary('<?php echo $tab; ?>');

    // Xử lý sự kiện nhấp vào tab
    document.querySelectorAll('#vocab-tabs .nav-link').forEach(tab => {
        tab.addEventListener('click', function(e) {
            e.preventDefault();
            const tabName = this.getAttribute('data-tab');
            loadVocabulary(tabName);
        });
    });
    ...
    // Hàm tải dữ liệu từ vựng bằng AJAX
    function loadVocabulary(tab) {
        fetch(`index.php?action=getVocabulary&tab=${tab}`)
            ...
            .then(data => {
                ...
                vocabList.innerHTML = '';
                if (data.length === 0) {
                    vocabList.innerHTML = '<p>No vocabulary items found for this level.</p>';
                } else {
                    data.forEach(item => {
                        const cardContent = `
                            <div class="card">
                        ...
                        const card = isLoggedIn ? `
                            <div class="col">
                                <a href="index.php?page=detail&type=vocabulary&title=${encodeURIComponent(item.title)}" style="text-decoration: none; color: inherit;">
                                    ${cardContent}
                                </a>
                            </div>
                        ` : `
                        ...
                        `;
                        vocabList.insertAdjacentHTML('beforeend', card);
                    });
                }

                // Cập nhật trạng thái tab
                document.querySelectorAll('#vocab-tabs .nav-link').forEach(t => {
            ...
            .catch(error => {
                ...
                document.getElementById('vocab-list').innerHTML = '<p>Error loading vocabulary items.</p>';
            });
    }
});
</script>
