...
<head>
  ...
  <link rel="stylesheet" href="public/css/style4Listen.css">
...
<body>
  ...
  <div class="container py-4">
    <div class="mb-4">
        <p id="title-vocabulary" style="color:rgba(99, 99, 99, 0.662)">Listening</p>
        <h3><img src="public/img/listen.png" alt="headphones" class="me-2"/>Vietnamese Listening</h3>
        <p>Whether you're a beginner or looking to improve your Vietnamese listening skills, we provide easy-to-follow lessons, interactive exercises, and practical audio to help you learn effectively.</p>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <ul class="nav nav-tabs border-0" id="listen-tabs">
            <li class="nav-item">
    ...
    <div class="row g-4">
        <div class="container my-5">
            <!-- Danh sách bài nghe sẽ được cập nhật động -->
            <div class="row row-cols-1 row-cols-md-3 g-4" id="listen-list">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Tải dữ liệu ban đầu
    loadListening('<?php echo $tab; ?>');

    // Xử lý sự kiện nhấp vào tab
    document.querySelectorAll('#listen-tabs .nav-link').forEach(tab => {
        tab.addEventListener('click', function(e) {
            e.preventDefault();
            const tabName = this.getAttribute('data-tab');
            loadListening(tabName);
        });
    });
    ...
    // Hàm tải dữ liệu bài nghe bằng AJAX
    function loadListening(tab) {
        fetch(`index.php?action=getListening&tab=${tab}`)
            .then(response => response.json())
            .then(data => {
                // Cập nhật danh sách bài nghe
                const listenList = document.getElementById('listen-list');
                listenList.innerHTML = '';
                if (data.length === 0) {
                    listenList.innerHTML = '<p>No listening items found for this level.</p>';
                } else {
                    data.forEach(item => {
                        const cardContent = `
                            <div class="card">
                        ...
                        const card = isLoggedIn ? `
                            <div class="col">
                                <a href="index.php?page=detail&type=listening&title=${encodeURIComponent(item.title)}" style="text-decoration: none; color: inherit;">
                                    ${cardContent}
                                </a>
                            </div>
                        ` : `
                        ...
                        `;
                        listenList.insertAdjacentHTML('beforeend', card);
                    });
                }

                // Cập nhật trạng thái tab
                document.querySelectorAll('#listen-tabs .nav-link').forEach(t => {
            ...
            .catch(error => {
                console.error('Error loading listening items:', error);
                document.getElementById('listen-list').innerHTML = '<p>Error loading listening items.</p>';
            });
    }
});
</script>
